 ...
 * Loop policies.
 */
template <int32_t BLOCK_THREADS, DataArrangement ArrangementT>
struct ThreadLoop
{
...
#pragma unroll
    for (int32_t THREAD_ITEM = 0; THREAD_ITEM < ITEMS_PER_THREAD; ++THREAD_ITEM)
    {
      if (ComputeBlockItem<BLOCK_THREADS, ITEMS_PER_THREAD, ArrangementT>(THREAD_ITEM) < num_items)
      {
        op(THREAD_ITEM);
      }
    }
...
#pragma unroll
    for (int32_t THREAD_ITEM = 0; THREAD_ITEM < ITEMS_PER_THREAD; ++THREAD_ITEM)
    {
      op(ComputeBlockItem<BLOCK_THREADS, ITEMS_PER_THREAD, ArrangementT>(THREAD_ITEM));
    }
  }
...
#pragma unroll
    ...
    {
      int32_t BLOCK_ITEM =
        ComputeBlockItem<BLOCK_THREADS, ITEMS_PER_THREAD, ArrangementT>(THREAD_ITEM);
      if (BLOCK_ITEM < num_items)
      {
        op(BLOCK_ITEM);
      }
    }
...
#pragma unroll
    for (int32_t THREAD_ITEM = 0; THREAD_ITEM < ITEMS_PER_THREAD; ++THREAD_ITEM)
    {
      op(THREAD_ITEM, ComputeBlockItem<BLOCK_THREADS, ITEMS_PER_THREAD, ArrangementT>(THREAD_ITEM));
    }
  }
 ...
 */
template <int32_t ITEMS_PER_THREAD, typename LoopPolicy, LoopType LoopT>
struct LoopExecutor
{};
