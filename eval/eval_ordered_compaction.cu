...
constexpr int32_t min_num_input                             = 1 << 17;
...
void Sweep(int32_t max_inverse_selectivity,
           int32_t max_num_input,
           bool fix_inverse_selectivity,
           bool fix_num_input)
{
  // Initialize output buffers
  thrust::device_vector<int32_t> row_ids_out_sorted_1(max_num_input);
  thrust::device_vector<int32_t> stencil_out_sorted_1(max_num_input);
  thrust::device_vector<int32_t> row_ids_out_sorted_2(max_num_input);
  thrust::device_vector<int32_t> row_ids_out_unsorted_1(max_num_input);
  thrust::device_vector<int32_t> stencil_out_unsorted_1(max_num_input);
  thrust::device_vector<int32_t> row_ids_out_unsorted_2(max_num_input);

  // Initialize row ids, stencil
  thrust::device_vector<int32_t> row_ids_in(max_num_input);
  thrust::sequence(row_ids_in.begin(), row_ids_in.end(), 0);
  thrust::device_vector<int32_t> stencil_in(max_num_input);
  thrust::host_vector<int32_t> stencil_in_host(max_num_input);
  ...
  // Sweep input size
  int32_t starting_input_size = fix_num_input ? max_num_input : min_num_input;
  for (int32_t num_input = starting_input_size; num_input <= max_num_input; ++num_input)
  {
    std::cout << "NUM INPUT: " << num_input << "\n";
    ...
    for (auto inverse_selectivity : possible_inverse_selectivities)
    {
      ...
      // Check that we are at the correct inverse selectivity
      if (fix_inverse_selectivity && inverse_selectivity != max_inverse_selectivity)
      {
        break;
      }
...
int main(int argc, char** argv)
{
  ...
  int32_t max_num_input           = 1 << 22;
  CommandLineArgs args(argc, argv);
  args.GetCmdLineArgument("mis", max_inverse_selectivity);
  args.GetCmdLineArgument("mni", max_num_input);
  bool fix_inverse_selectivity = args.CheckCmdLineFlag("fis");
  bool fix_num_input           = args.CheckCmdLineFlag("fni");
  ...
  if (args.CheckCmdLineFlag("help"))
  {
    std::cout << argv[0]
              ...
              << "\n\t[--mni=<max num input tuples for compaction>]\n";
    return 0;
  }

  // Sweep selectivities
  Sweep(max_inverse_selectivity, max_num_input, fix_inverse_selectivity, fix_num_input);
